...
class MyUserTestCase(TestCase):
    ...
    def setUp(self):
        self.test_user = MyUser.objects.create_user(email=self.email, first_name=self.first_name, last_name=self.last_name)
    ...
    def test_user_get_name(self):
        ...
        self.assertEqual(self.test_user.get_full_name(), self.first_name + ' ' + self.last_name)
...
class MyUserManager(TestCase):
    ...
    def test_create_user(self):
        test_user = MyUser.objects.create_user(email=self.email, first_name=self.first_name, last_name=self.last_name)
        ...
        with self.assertRaises(ValueError):
            MyUser.objects.create_user(email='', first_name=self.first_name, last_name=self.last_name)

        with self.assertRaises(ValueError):
            MyUser.objects.create_user(email=None, first_name=self.first_name, last_name=self.last_name)


    def test_create_superuser(self):
        self.email = 'superuser@test.test'
        test_user = MyUser.objects.create_superuser(email=self.email, password=self.password, first_name=self.first_name, last_name=self.last_name)
        self.assertEqual(MyUser.objects.all()[0], test_user)
        ...
        with self.assertRaises(ValueError):
            MyUser.objects.create_user(email='', password=self.password, first_name=self.first_name, last_name=self.last_name)


class ProfileTestCase(TestCase):

    def setUp(self):
        self.test_user = MyUser.objects.create_user(email='test@test.test', first_name='Harley', last_name='Quinn')
    ...
    def test_profile_follows(self):
        test_profile = Profile.objects.all()[0]
        friend = MyUser.objects.create_user(email="test@local.test", first_name="test", last_name="person")
